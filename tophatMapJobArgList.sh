...
#$solexa_quality_flag
command="tophat  $butterfly_search_flag $microexon_search_flag $no_novel_juncs_flag --mate-inner-dist $mate_inner_dist --mate-std-dev $mate_std_dev --min-anchor-length $min_anchor_length --min-isoform-fraction $min_isoform_fraction --num-threads $num_threads --max-multihits $max_multihits $GFF_flag $raw_junc_flag $other_options --output-dir $sampleOutputDir $BOWTIE_INDEXES/$ebwt_base $lfilelist $rfilelist >> $stdout_file 2>> $stderr_file"
